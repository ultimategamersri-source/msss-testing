...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // First visit — open chat AFTER intro screen finishes
  const hasVisited = localStorage.getItem("brightly_visited");
  if (!hasVisited) {
    localStorage.setItem("brightly_visited", "1");
    chatWindow.classList.add("hidden");
    // Wait for intro to finish, then open with fireworks
    window.addEventListener("introDone", () => {
      setTimeout(() => openChat(true), 300);
    }, { once: true });
    // Fallback: if intro never fires (e.g. no intro on page), open after 5s
    setTimeout(() => {
